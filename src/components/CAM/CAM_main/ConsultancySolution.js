...
useEffect(() => {
  const fetchConsultationById = async () => {
    try {
      toggleLoading(true);
      const response = await apiService.get(`/cam/consultation/get-consultid/${consultId}`);
...
if (!consultation) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      toggleLoading(true);
      const response = await apiService.put(`/cam/consultation/update-solutionbyid/${consultId}`, { 
        solution: solution 
      });
