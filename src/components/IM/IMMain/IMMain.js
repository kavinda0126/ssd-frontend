...
function IMMain({ toggleLoading }) {
  ...
  const handleButtonClick = (item) => {
    setShowModal(true);
    setproduct(item)
        apiService.delete(`/Product/deletequantity/${item._id}`)
              .then((response) => {
                alert("Deleted quantity record:");
              })
              .catch((error) => {
                console.error(error);
              })
  ...
  const handleAddQuantity = (up) => {
    ...
    if(type == "lubricant"){
      ...
      apiService.get(`/Product/lubricantstock`)
        .then((res) => {
          const lubricant = res
        ...
        const pastItemId = lubricanttoupdate ? lubricanttoupdate._id : null;
          ...
          apiService
      .put(`/Product/updatelubricant/${pastItemId}`, upproduct)
      .then((response) => {
      ...
      .then((res) => {
        ...
        apiService
      .put(`/Product/updateTire/${pastItemId}`, upproduct)
      .then((response) => {
        window.location.reload();
      })
      .catch((error) => {
