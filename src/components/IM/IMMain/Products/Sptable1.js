    ...
    useEffect(() => {
      ...
      const handleReject = (id, email,Image) => {
        ...
        if (shouldDelete) {
          ...
          apiService
            .post(
              `/Product/sendrejectemail`,
              emailData
            )
            .then((response) => {
              alert("Email sent successfully:");
              apiService
                .delete(`/Product/deletependingsp/${id}`,{ data: { image } })
                .then((response) => {
                  setShowModal(false);
      ...
      apiService.post(`/Product/addapprovedsp`,formData).then((res) =>{
          const newOrderId = res.order.orderId;
          ...
          apiService
            ...
            .then((response) => {
              ...
              apiService
                .delete(`/Product/removependingsp/${order._id}`)
                .then((response) => {
                  setShowModal(false); 
                  window.location.reload(); 
                })
