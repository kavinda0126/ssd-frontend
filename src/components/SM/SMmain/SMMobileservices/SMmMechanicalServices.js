...
const SMmMechanicalService = ({toggleLoading}) => {
  ...
  const assignTechnician = () => {
    ...
    apiService.put(`/api/mobile/update-mechanical/${selectedRequest._id}`, { 
  ...
  const getMechanicalRequests = () => {
      toggleLoading(true);
    apiService.get(`/api/mobile/get-mechanical`)
      .then((res) => {
  ...
  const deleteRequest = (id) => {
    ...
    if (shouldDelete) {
        toggleLoading(true); // Set loading to true before API call
      apiService.delete(`/api/mobile/delete-mechanical/${id}`)
        .then(response => {
