...
const Shedules = ({ toggleLoading }) => {
  ...
  function sendata(e) {
    ...
    toggleLoading(true);
    apiService.post(`/api/vehicle/add-serviceReqApp`, newservicerequest)
      .then(() => {
  ...
  function sendataToCompletedHistort() {
    ...
    apiService.post(`/appointment/addcompletedappointment`, newcompletedAppointment)
      .then(() => {
  ...
  const handleDateClick = async (date) => {
    try {
      ...
      toggleLoading(true); 
      const response = await apiService.get(`/appointment/get-acceptedappointmentbyDate/${formattedDate}`);
