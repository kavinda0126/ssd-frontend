...
import { createSafeElement } from "../../../utils/domSanitizer";
...
function EmployeeDetails({ toggleLoading }) {
  ...
  const generatePDF = () => {
    ...
    const addressP = createSafeElement("p", "323/1/A Main Street Battaramulla", { 
      class: "mb-1" 
    });
    contactDiv.appendChild(addressP);
    
    const emailP = createSafeElement("p", "", { class: "mb-1" });
    const emailIcon = createSafeElement("i", "", { 
      class: "uil uil-envelope-alt me-1" 
    });
    ...
    const phoneIcon = createSafeElement("i", "", { 
      class: "uil uil-phone me-1" 
    });
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      // Send DELETE request to backend API using CSRF-protected service
      await apiService.delete(`/api/hr/archive-employee/${_id}`);
  ...
  const handleCredUpdate = async () => {
    ...
    // Send email using CSRF-protected API service
    toggleLoading(true);
    await apiService.post("/api/hr/email", {
      to: emailOptions.to,
