...
const EmpFinance = ({toggleLoading}) => {
  ...
  const handleApprove = async (entry) => {
    ...
    if (confirmApproval) {
      try {
        await apiService.patch(`/api/finance/salarylist/updatestatus/${entry._id}`, { status: 'approved' });
  ...
  const handleReject = async (entry) => {
    ...
    if (confirmReject) {
      try {
        await apiService.patch(`/api/finance/salarylist/updatestatus/${entry._id}`, { status: 'rejected' });
  ...
  const sendSalaryPayments = async (entry) => {
    try {
      ...
      const response = await apiService.post("/api/finance/expenses/add-expense", {
  ...
  const updateBenefitProfiles = async (salaries) => {
    try {
      const response = await apiService.post("/api/finance/empbenefits/updatebenefits", { salarylist: salaries });
      if (response.ok) {
