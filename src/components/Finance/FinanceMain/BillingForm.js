    ...
    try {
      ...
      if (formData.paymentInvoiceId.substring(0, 2) === "PB") {
        ...
        await apiService.patch(
          `/api/finance/addsppaymentid/${formData.serviceRecordId}`,
          { paymentId: formData.paymentInvoiceId } // Corrected here
        );
      } else if (formData.paymentInvoiceId.substring(0, 2) === "PA") {
        // Send a PATCH request to add product payment ID
        await apiService.patch(
          `/api/finance/addproductpaymentid/${formData.serviceRecordId}`,
          { paymentId: formData.paymentInvoiceId } // Corrected here
        );
      } else {
        alert("Not PA or PB");
      ...
      alert("Error updating payment ID in service record");
    }

    try {
      const response = await apiService.post(
        `/api/finance/billing/createbill`,
        formData
      );
