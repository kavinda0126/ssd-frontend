...
const PendingPayments = ({ toggleLoading }) => {
  ...
  const sendUpdatetoInventory = async (paymentId) => {
    try {
      ...
      await apiService.patch(`/api/finance/updateinventory/${paymentId}`, {});
    } catch (error) {
  ...
  const markAsCompleted = async (paymentId) => {
    try {
      const response = await apiService.patch(`/api/finance/billing/inpersonpayment/${paymentId}`, { status: "completed" });
      if (!response.ok) {
  ...
  const markAsCancelled = async (paymentId) => {
    try {
      const response = await apiService.patch(`/api/finance/billing/inpersonpayment/${paymentId}`, { status: "cancelled" });
      if (!response.ok) {
        throw new Error("Failed to mark payment as completed");
      }
      fetchPendingPayments();
