...
const ProductSales = ({ toggleLoading }) => {
  ...
  const handleApprove = (order) => {
    apiService
      .put(
        `/Product/updatetoongoing/${order._id}`,
        { status: "ongoing" }
      )
      .then((res) => {
        getPendingOrder();
  ...
  const productApprove = (order) => {
    apiService
      .put(
        `/Product/updatetocompletedorder/${order._id}`,
        { status: "completed" }
      )
      .then((res) => {
