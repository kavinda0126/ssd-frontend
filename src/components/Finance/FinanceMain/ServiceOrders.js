...
const ServiceOrders = ({toggleLoading}) => {
  ...
  useEffect(() => {
    const fetchAllServiceOrders = async () => {
        try {
          toggleLoading(true)
            const response = await apiService.get(`/api/finance/service-record/all`);
            setAllServiceOrders(response);
        } catch (error) {
    ...
    const fetchPendingServiceOrders = async () => {
        try {
          toggleLoading(true)
            const response = await apiService.get(`/api/finance/service-record/pending`);
            setPendingServiceOrders(response);
        } catch (error) {
  ...
  const confirmMarkAsCompleted = async () => {
    try {
      await apiService.patch(
        `/api/finance/service-record/update/${orderIdToDelete}`,
        { price: 'Rs. 2000.00' }
      );
