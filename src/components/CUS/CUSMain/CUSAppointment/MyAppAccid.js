...
function MyAppAccid ({ toggleLoading }) {
  ...
  const getAccidentalData = async (userId) => {
    try {
      toggleLoading(true);
      const response = await apiService.get(`/appointment/get-accidentalAppointmentbyuserId/${userId}`);
      setaccidentalAppointment(response.data || response || []);
  ...
  const handleUpdateAppointment = () => {
    ...
    if (selectedAppointment && appointmentdate) { // Ensure appointmentdate is not empty
      ...
      toggleLoading(true); 
      apiService.put(`/appointment/update-accidentalAppointment/${selectedAppointment._id}`, {
