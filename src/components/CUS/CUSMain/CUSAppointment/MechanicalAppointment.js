...
function MechanicalAppointment({ toggleLoading }) {
  ...
  function sendata(e) {
    ...
    toggleLoading(true);
    apiService
      .post(`/appointment/addmechanicalAppointment`, {
  ...
  const fetchAvailableTimes = async (date) => {
    try {
      ...
      const response = await apiService.get(
        `/appointment/get-acceptedmechanicalappointmentbyDate/${formattedDate}`
      );
      ...
      // Handle any error (including 404) by showing all available times
      const allTimes = [
        "9.00am",
        "10.30am",
        "12.00pm",
    ...
    <div style={{ marginTop: "2%", marginLeft: "3%" }}>
      <div style={{ flex: "1", marginRight: "6%" }}>
        <h2 className="Appheading">
          Make an Appointment for Mechanical Repairs
        </h2>
        <Row>
          <Col md={7}>
            <Form onSubmit={sendata}>
              ...
              >
                <Form.Check
                  type="checkbox"
                  label="By scheduling your appointment you agree to accept our terms and regulations"
                  id="invalidCheck"
                  required
                />
          ...
          <Col md={4}>
            <img
              className="periodicalA-img"
              src={mechanicalrepairs}
              alt="Periodical Appointment"
