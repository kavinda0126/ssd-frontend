...
function MyApPer({ toggleLoading }) {
    ...
    const getPeriodicalData = async (userId) => {
        try {
            toggleLoading(true);
            const response = await apiService.get(`/appointment/get-periodicalAppointmentbyuserId/${userId}`);
            setperiodicalAppointment(response.data || response || []);

        } catch (error) {
            console.error('Error fetching appointments:', error);
            setperiodicalAppointment([]); // Set empty array on error
        }finally {
    ...
    const handleUpdateAppointment = () => {
        ...
        if (selectedAppointment && appointmentdate) { // Ensure appointmentdate is not empty
            ...
            toggleLoading(true); 
            apiService.put(`/appointment/update-periodicalAppointment/${selectedAppointment._id}`, {
    ...
    const Delete = (id) => {
        toggleLoading(true); 
        apiService.delete(`/appointment/delete-periodicalAppointment/${id}`)
            .then(response => {
    ...
    const fetchAvailableTimes = async (date) => {
        try {
            ...
            const response = await apiService.get(`/appointment/get-acceptedperiodicalappointmentbyDate/${formattedDate}`);
